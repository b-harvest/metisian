...
function updateTable(status) {
    ...
    for (let i = 0; i < status.Status.length; i++) {
        ...
        r.insertCell(1).innerHTML = `<div>${_.escape(status.Status[i].name)} (${_.escape(status.Status[i].address)})</div>`
        ...
        if (status.Status[i].name === "not connected") {
            r.insertCell(3).innerHTML = `<div class="uk-text-warning">${_.escape(status.Status[i].name)}</div>`
            bonded = "unknown"
        } else {
            r.insertCell(3).innerHTML = `<div class='uk-text-truncate'>${_.escape(status.Status[i].name.substring(0,24))}</div>`
        }
