...
func Serve(port string, updates chan *SequencerStatus, logs chan LogMessage, hideLogs bool) {
	...
	status := make(map[string]*SequencerStatus)
	...
	cast.Discard()
	log.Fatal(errors.New("metisian dashboard server failed" + err.Error()))
}
...
func (ch CacheHandler) ServeHTTP(writer http.ResponseWriter, request *http.Request) {
	...
	writer.Header().Set("X-Powered-By", "https://github.com/b-harvest/metisian")
